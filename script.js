...
(function(){
  ...
  document.querySelector('#gmailOpen')?.addEventListener('click',()=>{
    const s=modal.querySelector('.subject').value||'';
    const url='https://mail.google.com/mail/?view=cm&to='+encodeURIComponent(email)+'&su='+encodeURIComponent(s);
    window.open(url,'_blank');
  });
  document.querySelector('#copyEmail')?.addEventListener('click',()=>navigator.clipboard.writeText(email));
})();